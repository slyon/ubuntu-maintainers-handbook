...
bug reports are aggregated into a `<something>.crash` file.
...
Often there are also secondary libraries (**libs**) or dependencies (**deps**)
with more symbols (**syms**) that we'll need. This examines the binary for other
possible debug symbols (**dbgsyms**):
...
> Note: `apport-retrace` complains if the package isn't specified, but Apport
...
programming error. This can arise in code that neglects to create or initialise
...
For efficiency, compilers can skip initialising new pointers to zero, leaving
